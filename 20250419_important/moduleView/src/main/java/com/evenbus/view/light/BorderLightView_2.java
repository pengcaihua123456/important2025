...
public class BorderLightView_2 extends View {
    ...
    private int[] lightColors = {Color.RED, Color.YELLOW, Color.GREEN, Color.BLUE, Color.MAGENTA, Color.RED};
    private float[] lightPositions = {0f, 0.2f, 0.4f, 0.6f, 0.8f, 1f}; // 彩虹色分布，便于观察旋转
    private SweepGradient sweepGradient;
    private Matrix gradientMatrix;
    private float pathLength = 0f;
    private float segmentLength = 0f; // 光段的长度
    private float segmentRatio = 0.125f; // 光段占路径总长度的比例，默认1/8
    private float centerX, centerY; // SweepGradient中心点
    private long animationDuration = 3000L; // 动画持续时间（毫秒）

    public BorderLightView_2(Context context) {
        this(context, null);
    }

    public BorderLightView_2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BorderLightView_2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void updateGradient() {
        ...
        if (sweepGradient == null) {
            sweepGradient = new SweepGradient(centerX, centerY, lightColors, lightPositions);
        } else {
            // 重新设置中心点
            sweepGradient = new SweepGradient(centerX, centerY, lightColors, lightPositions);
        }
        ...
        gradientMatrix.postRotate(progress * 360, centerX, centerY);
        sweepGradient.setLocalMatrix(gradientMatrix);
        
        lightPaint.setShader(sweepGradient);
    }
