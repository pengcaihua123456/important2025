...
public class BorderLightView_1_3 extends View {
    ...
    private void init() {
        ...
        shadowPaint.setStrokeWidth(borderWidth * 2f); // 更宽一些以增强发光效果
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 创建内侧裁剪路径（向内偏移半个边框宽度，使发光效果在边框内部）
        innerPath.reset();
        float innerOffset = borderWidth / 2; // 向内偏移半个边框宽度
        RectF innerRect = new RectF(
                borderRect.left + innerOffset,
                borderRect.top + innerOffset,
                borderRect.right - innerOffset,
                borderRect.bottom - innerOffset
        );
        // 圆角半径也相应减小，但确保不小于0
        float innerCornerRadius = Math.max(0, cornerRadius - innerOffset);
    ...
    private void updateGradient() {
        ...
        // 为发光效果设置相同的渐变
        shadowPaint.setShader(new LinearGradient(startX, startY, endX, endY, lightColors, lightPositions, android.graphics.Shader.TileMode.CLAMP));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (pathMeasure != null && pathLength > 0) {
            ...
            // 裁剪到内侧路径，使发光效果仅显示在边框内部
            canvas.clipPath(innerPath);
            
            // 绘制发光效果（使用相同渐变和模糊效果）
            canvas.drawPath(lightSegmentPath, shadowPaint);
            
            // 恢复画布状态
            canvas.restoreToCount(saveCount);
            
            // 再绘制清晰的流光段（不受裁剪影响，绘制在边框上）
