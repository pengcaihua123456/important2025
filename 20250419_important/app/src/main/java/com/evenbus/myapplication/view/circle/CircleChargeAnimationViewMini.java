...
public class CircleChargeAnimationViewMini extends View {
    // 圆环参数
    private Paint mRingPaint;
    private RectF mRingRect;
    private int[] mRingColors = {Color.RED, Color.GREEN, Color.BLUE};
    private float mRingWidth = 30f;
    ...
    private static final long PARTICLE_INTERVAL = 100; // 每100ms生成一个粒子
    ...
    public CircleChargeAnimationViewMini(Context context) {
        super(context);
        init();
    }

    public CircleChargeAnimationViewMini(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 计算圆环半径
        mRingRadius = Math.min(w, h) * 0.4f - mRingWidth/2;
    ...
    private void generateParticles() {
        ...
        if (currentTime - mLastParticleTime > PARTICLE_INTERVAL && mParticles.size() < MAX_PARTICLES) {
            ...
            // 目标位置在圆环内边缘
            float targetX = mCenterX + (float) Math.cos(Math.toRadians(angle)) * (mRingRadius - mRingWidth/2);
            float targetY = mCenterY + (float) Math.sin(Math.toRadians(angle)) * (mRingRadius - mRingWidth/2);

            // 随机大小 (最大不超过圆环宽度)
            float startSize = mRandom.nextFloat() * mRingWidth/3 + 2;
            float targetSize = mRingWidth * 0.8f; // 最终接近圆环宽度
